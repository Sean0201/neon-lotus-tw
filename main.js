...
const CAT_LABELS = {
  ...
  OTHER:       { tw: '其他',   en: 'OTHER' },
};
const CAT_ORDER = ['TOPS','TEES','LONGSLEEVES','SHIRTS','POLOS','TANKS','SWEATERS','JERSEYS','OUTERWEAR','JACKETS','HOODIES','BOTTOMS','PANTS','SHORTS','SKIRTS','DRESSES','SETS','BAGS','CAPS','ACCESSORIES','FOOTWEAR','UNDERWEAR','OTHER'];
...
function calcPrice(vnd, tag) {
  ...
  const twd_carryback = Math.round(base_ntd / 10) * 10;                // 四捨五入到 10
  const twd_shipping  = psychPrice(base_ntd + ship_ntd);               // 50/90 結尾
