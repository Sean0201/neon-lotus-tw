...
function computeCart(args) {
   ...
   *   groups 只代表 regular_items,promo_groups 獨立呈現。
   */
  let raw_subtotal = 0;
  ...
  return {
    ...
    raw_subtotal,         // 整車 (regular + promo)
    regular_subtotal,     // 正常品 (參與折扣的部分)
    promo_subtotal,       // 特價品 (不參與任何折扣)
    groups,               // 僅 regular
    promo_groups,         // 僅 promo
    ...
    regular_final,        // regular 部分跑完 pipeline + floor clamp 後
    final_subtotal,       // = regular_final + promo_subtotal
