...
// V15: 靜態快照架構 (scripts/dump-snapshot.js + /data/products.json + /data/gallery/*)。
//      前台優先打 CDN 上的 JSON,失敗才 fallback 打 Supabase。
//      cache 結構未變,但升 V15 強制清舊 cache,避免 stale 資料混淆偵錯。
const CACHE_KEY     = 'NEON_LOTUS_TW_V15';
const CACHE_TS_KEY  = 'NEON_LOTUS_TW_V15_TS';
...
// ── Static snapshot config ────────────────────────────────────
// Snapshot 由 scripts/dump-snapshot.js 於 Vercel build 時產出到 /data/。
// 前台載入順序:snapshot → Supabase(fallback)。降低 90%+ Supabase egress。
const SNAPSHOT_CORE_URL    = '/data/products.json';
const SNAPSHOT_GALLERY_URL = brandId => `/data/gallery/${encodeURIComponent(brandId)}.json`;

async function _fetchSnapshot(url) {
  // cache: 'default' 讓瀏覽器走 HTTP cache header(Vercel 設 max-age=60 s-maxage=300 SWR=1d)
  const r = await fetch(url, { cache: 'default' });
  if (!r.ok) throw new Error(`snapshot ${url} → ${r.status}`);
  return r.json();
}
...
/**
 * Tiered localStorage cache write.
 * 6000+ 商品時完整 payload 會超過 5MB → 逐層剝欄位重試。
 * 兩個入口(snapshot 路徑 + Supabase fallback 路徑)共用同一份邏輯。
 */
function _writeCoreCache(data) {
  const tiers = [
    () => data,
    () => ({
      ...data,
...
async function loadSupabaseData() {
  ...
  try {
    ...
    if (cached && cachedTs && (Date.now() - Number(cachedTs)) < CACHE_TTL) {
      ...
      if (data && data.brands && data.products) {
        ...
        // 靜態快照架構:featured / brand_promos 已在 snapshot 內,不再需要背景重抓,
        // 否則每個訪客都會多打 2 次 Supabase,失去 snapshot 省 egress 的意義。
        // Admin 改完 featured/promo 後按「重建快照」按鈕觸發 rebuild 即可。
        return;
      }
    }
  } catch (e) {
  ...
  // ── Try static snapshot (Vercel Edge CDN) ───────────────────
  // 90%+ 的訪客在此結束,不會打 Supabase。snapshot 由 admin 面板按鈕觸發 rebuild。
  try {
    const t0 = performance.now();
    const data = await _fetchSnapshot(SNAPSHOT_CORE_URL);
    if (data && data.brands && data.products) {
      const ms = Math.round(performance.now() - t0);
      console.log(`[supabase-client] Snapshot loaded in ${ms}ms — ` +
        `${data.brands.length} brands, ${data.products.length} products`);
      ...
      // Write to localStorage cache with existing tier-fallback logic
      _writeCoreCache(data);
      return;
    }
    console.warn('[supabase-client] Snapshot payload missing brands/products, falling back to Supabase');
  } catch (err) {
    console.warn('[supabase-client] Snapshot fetch failed, falling back to Supabase:', err?.message || err);
  }
...
async function loadBrandDetail(brandId) {
  ...
  try {
    const t0 = performance.now();
    const snap = await _fetchSnapshot(SNAPSHOT_GALLERY_URL(brandId));
    if (snap && (snap.gallery || snap.sizes)) {
      // lsoul URL 修補: 過期 /cache/<hash>/ thumbnail 路徑要剝掉
      const fixLsoul = (url) => {
        if (typeof url !== 'string' || !url.includes('lsoul.com/media/catalog/product/cache/')) return url;
        return url.replace(/\/cache\/[a-f0-9]+\//, '/');
      };
      const galleryByProduct = snap.gallery || {};
      const sizesByProduct   = snap.sizes   || {};

      let galleryCount = 0, sizesCount = 0;
      for (const p of data.products) {
        if (p.brand_id !== brandId) continue;
        const g = galleryByProduct[p.id] || [];
        if (brandId === 'lsoul') {
          for (const item of g) {
            item.url = fixLsoul(item.url);
            item.original_url = fixLsoul(item.original_url || item.url);
          }
        }
        p.images.gallery = g;
      ...
      const ms = Math.round(performance.now() - t0);
      console.log(`[supabase-client] Brand "${brandId}" detail from snapshot in ${ms}ms — ` +
        `${galleryCount} gallery images, ${sizesCount} sizes`);
      return;
    }
  } catch (err) {
    console.warn(`[supabase-client] Snapshot gallery/${brandId} failed, falling back to Supabase:`,
      err?.message || err);
  }
